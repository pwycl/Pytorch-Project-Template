...
class SMTDataLoader:
    ...
    def get_k_fold_indices(self, folds, len_dataset):
        ...
        for _, idx in skf.split(torch.zeros(len_dataset),
                                torch.zeros(len_dataset)):
    ...
    def k_fold_loader_generator(self, folds):
        ...
        for fold, (train_idx, val_idx, test_idx) in enumerate(
            ...
            train_loader = DenseDataLoader(
                dataset[train_idx], self.config.batch_size, shuffle=True)
            val_loader = DenseDataLoader(
                dataset[val_idx], self.config.batch_size, shuffle=False)
            test_loader = DenseDataLoader(
                dataset[test_idx], self.config.batch_size, shuffle=False)
