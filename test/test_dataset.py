...
def test_baseDataset_Loader(filted_dataset=None):
    ...
    for batch in loader:
        ...
        assert batch.num_nodes == sum(
            [data.num_nodes for data in dataset])  # 2788794
        assert batch.num_edges == sum(
            [data.num_edges for data in dataset])  # 13347768
...
def test_SMT():
    ...
    dataset = smt.get_dataset('SMT',
                              ...
                              dataset_div=config.dataset_div)
    ...
    for batch in smt.train_loader:
        ...
        assert list(batch.x.size())[1:] == [
            smt.num_nodes, dataset.data.num_features]  # [24, 5187, 20]
        assert list(batch.adj.size())[1:] == [
            smt.num_nodes, smt.num_nodes]  # [24,5187, 5187]
